...
% *
% * RPCBPROC_UADDR2TADDR(strint) RETURNS (arpc_addr_t)
% *	Returns the netbuf address from universal address.
% *
% * RPCBPROC_TADDR2UADDR(arpc_addr_t) RETURNS (string)
% *	Returns the universal address from arpc_addr_t address.
% *
...
% */
struct rpcb {
	arpcprog_t r_prog;		/* program number */
	arpcvers_t r_vers;		/* version number */
...
%extern "C" {
%#endif
%extern  axdr_ret_t axdr_rpcblist(axdr_state_t *, rpcblist**);
%#ifdef	__cplusplus
%}
%#endif
%
...
struct rpcb_rmtcallargs {
	arpcprog_t prog;		/* program number */
	arpcvers_t vers;		/* version number */
	arpcproc_t proc;		/* procedure number */
...
%		char *results_val;
%	} results;
%	axdrproc_t	xdr_res;	/* decodes results */
%};
#endif /* RPC_HDR */
%
%/*
...
%
%/*
% * We don't define arpc_addr_t in RPCL, Instead we merely declare the
% * XDR routine xdr_arpc_addr_t() here, and implement it ourselves in 
% * rpc/rpcb_prot.c.
% */
%#ifdef __cplusplus
%extern "C" axdr_ret_t axdr_arpc_addr_t(axdr_state_t *, arpc_addr_t *);
%
%#else /* __STDC__ */
%extern axdr_ret_t axdr_arpc_addr_t(axdr_state_t *, arpc_addr_t *);
%
%#endif
#endif /* def RPC_HDR */
...
program RPCBPROG {
	version RPCBVERS {
		...
		string
		RPCBPROC_TADDR2UADDR(arpc_addr_t) = 8;
	} = 3;

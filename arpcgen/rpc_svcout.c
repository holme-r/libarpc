...
static char HANDLER[] = "handler";
static char INPROC[] = "inproc";
static char OUTPROC[] = "outproc";
static char INLEN[] = "inlen";
static char OUTLEN[] = "outlen";
...
static void write_lookup( definition *, char * );
...
{
	f_print(fout, "\t\t*%s = (axdrproc_t) axdr_%s;\n",
...
write_rest()
{
	f_print(fout, "\n");
	if (inetdflag) {
		f_print(fout, "\tif (%s == (ar_svc_xprt_t *)NULL) {\n", TRANSP);
	...
	char *storage;
{
	...
	/* write out lookup function for each program */
	for (l = defined; l != NULL; l = l->next) {
		def = (definition *) l->val;
		if (def->def_kind == DEF_PROGRAM) {
			write_lookup(def, mtflag ? "" : storage);
		}
	}

	if (lookupflag) {
		/* no need to write dispatch function */
		return;
	}
	...
	definition *def;
{
	...
	for (vp = def->def.pr.versions; vp != NULL; vp = vp->next) {
		for (proc = vp->procs; proc != NULL; proc = proc->next) {
			...
			if (mtflag) {
				f_print(fout, " *argp, void *%s, ar_svc_req_t *%s)\n",
					RESULT, RQSTP);


			}
			else
				f_print(fout, " *argp, ar_svc_req_t *%s)\n",
					RQSTP);
...
static void
write_lookup(definition *def, char *storage)
{
	version_list *vp;
	proc_list *proc;
	...
	for (vp = def->def.pr.versions; vp != NULL; vp = vp->next) {
		...
		f_print(fout, "void\n");
		pvname_lookup(def->def_name, vp->vers_num);

		f_print(fout, "(u_int32_t %s, ", PROC);
		f_print(fout, "ar_svc_handler_fn_t *%s, ", HANDLER);
		f_print(fout, "axdrproc_t * %s, ", INPROC);
		f_print(fout, "axdrproc_t * %s, ", OUTPROC);
		f_print(fout, "int * %s, ", INLEN);
		f_print(fout, "int * %s)\n", OUTLEN);
		f_print(fout, "{\n");

		f_print(fout, "\tswitch (%s) {\n", PROC);
		for (proc = vp->procs; proc != NULL; proc = proc->next) {
			f_print(fout, "\tcase %s:\n", proc->proc_name);
			if (proc->arg_num < 2) { /* single argument */
				p_xdrfunc(INPROC, proc->args.decls->decl.type);
			} else {
				p_xdrfunc(INPROC, proc->args.argname);
			}
			p_xdrfunc(OUTPROC, proc->res_type);
			f_print(fout, "\t\t*%s = (ar_svc_handler_fn_t) ", 
				HANDLER);
			if (newstyle) {
			...
			if (proc->arg_num < 2) { /* single argument */
				...
				f_print(fout, "sizeof(%s);\n",
					proc->args.argname);
			}

			f_print(fout, "\t\t*%s = ", OUTLEN);
			if (mtflag) {
		...
		f_print(fout, "\t\t*%s = (axdrproc_t)0;\n", INPROC);
		f_print(fout, "\t\t*%s = (axdrproc_t)0;\n", OUTPROC);
		f_print(fout, "\t\t*%s = (ar_svc_handler_fn_t)NULL;\n", HANDLER);
		f_print(fout, "\t\t*%s = 0;\n", INLEN);
		f_print(fout, "\t\t*%s = 0;\n", OUTLEN);
		f_print(fout, "\t}\n");
...
write_program(definition *def, char *storage)
{
	...
	for (vp = def->def.pr.versions; vp != NULL; vp = vp->next) {
		...
		f_print(fout, "(ar_svc_req_t *%s, ", RQSTP);
		f_print(fout, " ar_svc_xprt_t *%s)\n", TRANSP);
		...
		f_print(fout, "\tint %s_len, %s_len;\n",
			ARG, RESULT);
		f_print(fout, "\taxdrproc_t axdr_%s, axdr_%s;\n",
			ARG, RESULT);
		if (mtflag)
			f_print(fout, "\tbool_t (*%s)(char *, "
				"void *, ar_svc_req_t *);\n",
				ROUTINE);
		else
			f_print(fout,
		...
		f_print(fout, "\t");
		pvname_lookup(def->def_name, vp->vers_num);
		f_print(fout, "(%s->rq_proc, (ar_svc_handler_fn_t *)&%s,\n"
			"\t\t&axdr_%s, &axdr_%s, &%s_len, &%s_len);\n", 
			RQSTP, ROUTINE, ARG, RESULT, ARG, RESULT);
		f_print(fout, "\tif (!%s) {\n", ROUTINE);
		if (!nullproc(vp->procs)) {
			f_print(fout, "\t\tif (%s->rq_proc == "
				"NULLPROC) {\n", RQSTP);
			f_print(fout,
				"\t\t\t(void) svc_sendreply(%s, axdr_void,\n\t\t\t\t\
(char *)NULL);\n",
				TRANSP);
			print_return("\t\t\t");
			f_print(fout, "\t\t}\n");
		...
		if (mtflag)
			f_print(fout,
				"\tif (%s > 0 && !svc_sendreply(%s, axdr_%s, (char *)&%s)) {\n",
				RETVAL, TRANSP, RESULT, RESULT);
		else
			f_print(fout,
				"\tif (%s != NULL && !svc_sendreply(%s, axdr_%s, %s)) {\n",
		...
		if (mtflag) {
			f_print(fout,"\tif (!");
			pvname(def->def_name, vp->vers_num);
			f_print(fout,"_freeresult(%s, axdr_%s, (caddr_t) &%s))\n",
...
printif(proc, transp, prefix, arg)
	...
	char *arg;
{
	f_print(fout, "\tif (!svc_%s(%s, axdr_%s, (char *)%s%s)) {\n",

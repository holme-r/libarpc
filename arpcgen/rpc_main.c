 ...
 */
static void addarg(char *cp);   /* add another argument to the list */
/* put argument at specified location  */
...
int mtflag = 1;		/* Support for MT */
int streamflag = 1;	/* Support for async streaming XDR */
int debugflag = 1;
int lookupflag = 1;	/* Support lookup only, no svc dispatch func */

#define INLINE 0
/* length at which to start doing an inline */

int rpc_inline = INLINE;
/*
...
int newstyle = 1;		/* newstyle of passing arguments (by value) */
	...
	char *outfile;
{
	...
	f_print(fout, "#include <libarpc/arpc.h>\n");
	...
	int netflag;
{
	...
	if ((logflag || inetdflag || pmflag || tirpcflag) && !nomain)
		...
		if (argv[i][0] != '-') {
			...
			for (j = 1; argv[i][j] != 0; j++) {
				...
				case 'P':
					...
					break;
				case 'p':
					/* 'pause-able' xdr conversion */
					streamflag = 1;
					break;
				case 'd':
					/* debug stringify support */
					debugflag = 1;
					break;
				case 'x':
					/* e'x'tended IO. Implies no dispatch
					 * and other changes...
					 */
					lookupflag = 1;
					break;
...
remote procedures\n");
	...
	f_print(stderr, "-x\t\t'e'x'tended IO. Implies no dispatch and other changes [Always On]\n");

...
static void get_type( int *, char **, char **, defkind );
	...
	definition *defp;
{
	...
	proc_list **ptailp;
	int num_args;
	int isvoid = FALSE;	/* whether first argument is void */
	int encap;
	...
	do {
		...
		do {
			...
			get_type(&encap, &plist->res_prefix, &plist->res_type,
				 DEF_PROGRAM);
			if (streq(plist->res_type, "opaque") || encap) {
	...
	defkind dkind;
{
	...
	get_type(&dec->encap, &dec->prefix, &dec->type, dkind);
	dec->rel = REL_ALIAS;
	if (streq(dec->type, "void")) {
		return;
	}
	...
	int num;  /* arg number */
{
	...
	get_type(&dec->encap, &dec->prefix, &dec->type, dkind);
	if (dec->encap) {
		error("no encapsulated arguments");
	}
	dec->rel = REL_ALIAS;
	if (peekscan(TOK_IDENT, &tok)) /* optional name of argument */
		snprintf(name, sizeof(name), "%s", tok.str);
	else
...
static void
get_type(encapp, prefixp, typep, dkind)
	int *encapp;
	char **prefixp;
	char **typep;
	defkind dkind;

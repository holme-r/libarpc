...
static void pasyncarglist( proc_list *, char * );
static void pprocdef( proc_list *, version_list *, char *, int );
static void pprocdefasync( proc_list *, version_list *);
...
void
print_xdr_func_def(name, pointerp)
char* name;
int pointerp;
{

	f_print(fout, "extern axdr_ret_t axdr_%s(axdr_state_t *, %s%s);\n", 
		name, name, pointerp ? "*" : "");
...
pfreeprocdef(char * name, char *vers)
{
	...
	pvname(name, vers);
	f_print(fout,"_freeresult(ar_svc_xprt_t *, axdrproc_t, caddr_t);\n");
...
pdispatch(char * name, char *vers)
{
	...
	pvname(name, vers);
	f_print(fout,
	    "(ar_svc_req_t *rqstp, ar_svc_xprt_t *transp);\n");
}


static void
plookup(char * name, char *vers)
{

	f_print(fout, "void ");
	pvname_lookup(name, vers);
	f_print(fout,
		"(u_int32_t, ar_svc_handler_fn_t *, \n\t"
	...
	definition *def;
{
	...
	for (vers = def->def.pr.versions; vers != NULL; vers = vers->next) {
		...
		f_print(fout, "extern  ");
		if (mtflag) {
			plookup(def->def_name, vers->vers_num);
		}
		if (!lookupflag) {
		...
		for (proc = vers->procs; proc != NULL;
			...
			f_print(fout, "extern  ");
			pprocdef(proc, vers, "ar_client_t *", 0);
			f_print(fout, "extern  ");
			pprocdef(proc, vers, "ar_svc_req_t *", 1);
			if (lookupflag) {
				pprocdefasync(proc, vers);
			}
		}
		if (!lookupflag) {
...
static void
pprocdef(proc, vp, addargtype, server_p)
	proc_list *proc;
	version_list *vp;
	char* addargtype;
	int server_p;
...
static void
pprocdefasync(proc_list *proc, version_list *vp)
{
	f_print(fout, "typedef void (*");
	pvname_cb(proc->proc_name, vp->vers_num);
	f_print(fout, ")(ar_clnt_call_obj_t cco, void *arg, ");
	f_print(fout, "const arpc_err_t *stat, ");
	ptype(proc->res_prefix, proc->res_type, 1);
	f_print(fout, "*);\n");
	...
	pasyncarglist(proc, vp->vers_num);
}
...
static void
pasyncarglist(proc_list *proc, char *vers)
{
	decl_list *dl;
